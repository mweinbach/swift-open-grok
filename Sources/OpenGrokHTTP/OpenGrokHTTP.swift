...
// MARK: - Retry-After & HTTP-Date Parsing

/// Parse a `Retry-After` header value (delta-seconds or HTTP-date).
///
/// Returns delta-seconds clamped to `>= 0`. If `value` is `nil`, malformed, or
/// in the past, returns `nil` (meaning fall back to the client's configured backoff).
...
/// Computes exponential backoff with full jitter matching the Rust reference.
